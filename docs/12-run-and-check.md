# 12 - Run it, and the demo checks

The README carries the quick start. This file is the full run guide: HTTPS on the laptop, the
phone, the flagged cloud upgrades, the milestone checks the team walks before a judged run, and
deploy. Every environment variable the repo reads is listed in `.env.example` with its usage.
...
instead, which is what the M0 and M1 checks below walk.
...
- The browser bundle reads no environment variable; keys stay in `.env.local` for the dev proxy
  (docs/09).
...
4. Tap I NEED HELP, then CALL 911. Allow the microphone. The panel header reads "Listening to
   you" while the agent is live, and the panel shows what it heard you say. "On the line"
   with the reply buttons showing means the script is answering: no agent id, mic refused, or
   no session within 4 s. The event log keeps the distinction for the debug screen.
...
6. A different coach persona: `ELEVENLABS_COACH_VOICE=Daniel` (a library name or a voice id)
   next to the key. The proxy resolves it once and logs the match; a miss keeps Brian and
   never substitutes quietly. The dispatcher voice is fixed.
...
3. No key, or no answer within six seconds: nothing happens, and the camera's own cues carry on.
4. The free tier rate-limits after roughly six calls a minute. Both routes return null on a
   429 and the app carries on, so space rehearsal runs out.

`node scripts/assess-frame.mjs photo.jpg [model]` runs the app's exact question on a photo,
which is how two models get compared before the judged run.
`?fake=1&flag=sceneAssess` demos the flow with a canned model: pick what it says in the fake
controls, then "Start over", since the frame goes out about a second into triage.

### The intent router

A bystander in a panic does not say "not breathing", they say "the poor man went down in the
hallway and he is grey". The local matcher misses that and so do the phrase cues, and then the
same model gets the sentence and the buttons that are on the screen right now (docs/04 item 8).
...
2. Once a state has been read the card shrinks to its caption so the camera shows the rescuer;
   tap the card to reopen or hide the picture.
3. Every spoken answer has a button under the card. NEXT always moves. "Ambulance is here"
   ends the machine and opens the handoff.
4. In compressions the metronome runs and the pill shows the live rate. Stop pushing for three
   seconds: "Don't stop." Cover the lens: the blind line, the beat continues.
5. In bleeding pressure, lift both hands: "Don't let go!" within about two seconds.
6. CALL 911 opens the simulated dispatcher; the SITREP lines answer it. The panel reads as a
   real call because LAUNCH already said it is not one.
7. The handoff shows the read-aloud block, the paramedic headline, the timeline and a QR.

`window.mayday.log.entries()` in the console lists what was heard and said.

## Deploy

Target is DigitalOcean App Platform, spec in `.do/app.yaml` (static site, `npm run build` to
